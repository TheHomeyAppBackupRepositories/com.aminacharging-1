...
async function initReadWriteCapability(device, zclNode, capabilityId, cluster, attributeName, reportParser = defaultReportParser, setParser = defaultSetParser, { minInterval = 0, maxInterval = 3600, minChange = 1, } = {}, endpointId) {
    const endpoint = endpointId ?? device.getClusterEndpoint(cluster) ?? 1;
...
async function initReadCommandCapability(device, zclNode, capabilityId, cluster, commandName, commandArgParser = defaultSetParser, attributeName, reportParser = defaultReportParser, { minInterval = 0, maxInterval = 3600, minChange = 1, } = {}, endpointId, pollInterval) {
    const endpoint = endpointId ?? device.getClusterEndpoint(cluster) ?? 1;
    // Retrieve initial value
    await readInitialValue(device, zclNode, capabilityId, cluster, attributeName, reportParser, endpoint);
...
async function initReadOnlyCapability(device, zclNode, capabilityId, cluster, attributeName, reportParser = defaultReportParser, { minInterval = 0, maxInterval = 3600, minChange = 1, } = {}, endpointId) {
    const endpoint = endpointId ?? device.getClusterEndpoint(cluster) ?? 1;
    // Retrieve initial value
    await readInitialValue(device, zclNode, capabilityId, cluster, attributeName, reportParser, endpoint);
    // Configure the capability
